...
"""
============================================================================
sync_prices.py v3.1 - סנכרון מחירים + בניית inverted index
============================================================================

שינויים גרסה 3.1:
  - INDEX_MAX_PRODUCTS_PER_TOKEN: 500 → 5000
    תיקון באג: מילים פופולריות בעברית ("חלב", "סוכר", "לחם") מופיעות
    ביותר מ-500 מוצרים ברשתות הגדולות והוסרו בטעות מהאינדקס. עם הסף
    החדש הן יישארו באינדקס. ההגנה מפני טוקנים פתולוגיים אמיתיים
    (תווים בודדים, רעש) נשמרת.
...
INDEX_MAX_PRODUCTS_PER_TOKEN = 5000   # ⬆️ הועלה מ-500 ל-5000 (v3.1)
                                      # מילים פופולריות בעברית (חלב/סוכר/לחם)
                                      # מופיעות ביותר מ-500 מוצרים. הסף הקודם
                                      # הסיר אותן בטעות וגרם לבאג חיפוש.
...
def main():
    logger.info('=' * 70)
    logger.info('מתחיל סנכרון מחירים v3.1 (kagglehub + inverted index)')
    logger.info('=' * 70)
